...
`pnpm typecheck && pnpm test` at root, `pytest` in `services/tts`. End-to-end: generate a story with Claude Code or Codex (never Ollama, see gotchas), approve it, watch the pipeline strip; a full real render costs ~$3–5 (fal + Gemini).
